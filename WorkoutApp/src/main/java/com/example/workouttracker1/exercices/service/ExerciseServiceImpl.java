...
import com.example.workouttracker1.exercices.repository.MuscleGroupRepository;
import com.example.workouttracker1.exercices.dto.ExerciseDTO;
import com.example.workouttracker1.exercices.entity.Exercise;
import com.example.workouttracker1.exercices.repository.ExerciseRepository;
...
public class ExerciseServiceImpl implements ExerciseService {
    ...
    public ExerciseDTO saveExercise(ExerciseDTO exerciseDTO) {
        muscleGroupRepository.save(convertToEntity(exerciseDTO).getMuscleGroup());
    ...
    public List<ExerciseDTO> getRandomExercises(Integer numOfExercises, String muscleGroup, Boolean isWithWeights) {
        ...
        //if cardio
        if(muscleGroup.equalsIgnoreCase(MuscleGroupEnum.CARDIO.name())) {
    ...
    private List<ExerciseDTO> setFirstTwoExercisesCardio(List<ExerciseDTO> tempResult) {
        for(int i=0; i<2; i++) {
            if (!tempResult.get(i).getMuscleGroupName().equals(MuscleGroupEnum.CARDIO.name())) {
                //find first cardio
    ...
    private List<Long> getAllIds(String muscleGroup, Boolean isWithWeights) {
        List<Long> allIds;
        if (muscleGroup.equalsIgnoreCase(MuscleGroupEnum.WHOLE_BODY.name())) {
            if(!isWithWeights) {
    ...
    @Override
    public List<ExerciseDTO> getExercisesForMuscleGroup(MuscleGroupEnum muscleGroup) {
        if (muscleGroup.name().equalsIgnoreCase(MuscleGroupEnum.WHOLE_BODY.name())) {
    ...
    private List<ExerciseDTO> getRandomCardioExercises(Integer numOfExercises, List<Long> randomIdsBeforeCardio) {
        List<Long> cardioIds = getAllIds(MuscleGroupEnum.CARDIO.name(), false);
